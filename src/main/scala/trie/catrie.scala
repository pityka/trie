...
class CANodeReader(backing: Reader) extends CNodeReader {
  ...
  def readPointers1(bb: ByteBuffer): Seq[(Byte, Long)] = {
    ...
    val elems: Seq[Byte] = 0 until 16 map (i => bb.get)
    val pointers = 0 until 16 map (i => bb.getLong)
    elems zip pointers take (elemSize)
  }

  def readPointers(bb: ByteBuffer): Seq[(Byte, Long)] = {
    val children = readPointers1(bb)
    val next = bb.getLong
    if (next >= 0) {
      val ar = Array.ofDim[Byte](153)
      backing.get(next, ar)
  ...
  def read(i: Long): Option[CNode] = {
    if (backing.size >= i + 4) {
      ...
      val children = {
        val pointers: Seq[(Byte, Long)] = readPointers(bb)
      ...
      val prefix = ar.slice(8 + 153, ar.size)
  ...
  def readAddress(i: Long, b: Byte): Long = {
    ...
    val pointers: Seq[(Byte, Long)] = readPointers(bb)
    assert(pointers.size <= 256)
    pointers.find(_._1 == b).map(_._2).getOrElse(-1L)
  }

  def readPartial(i: Long): PartialCNode = {
    val recordSize = backing.readInt(i, bufInt)
    val ar = Array.ofDim[Byte](recordSize - 153 - 8)
    backing.get(i + 4 + 8 + 153, ar)
...
class CANodeWriter(backing: Writer) extends CANodeReader(backing) with CNodeWriter {
  ...
  def writePointers(bb: ByteBuffer, pointers: Seq[(Byte, Long)]): Unit = {
    val (first16, rest) = pointers.splitAt(16)
  ...
  def write(n: CNode) = {
    val size = n.prefix.size + 4 + 8 + 153
  ...
  def updateRoute(old: CNode, b: Byte, a: Long) = {
    def update(b: Byte, a: Long, idx: Long): Unit = {
      ...
      if (elems.map(_._1).contains(b)) {
        ...
        backing.writeLong(a, idx + 1 + 16 + idx2 * 8, bufLong)
      } else {
        if (elems.size == 16) {
          if (next >= 0) update(b, a, next)
            ...
            backing.set(nidx, ar)
            backing.writeLong(nidx, idx + 1 + 16 + 16 * 8, bufLong)
          ...
          backing.writeLong(a, idx + 1 + 16 + elems.size * 8, bufLong)
        }
      }
    }
