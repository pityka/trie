...
class TrieSpec extends FunSpec with Matchers {
  ...
  def tests(newstorage: () => Writer, name: String) = {
    ...
    describe("cnode storage " + name) {
      it("simple") {
        ...
        val s = newstorage()
        val ns = new SimpleCNodeWriter(s)
        ns.append(n1)
        ns.read(0).get should equal(n1)
    ...
    describe("cnode adaptive storage " + name) {
      it("simple") {
        val n1 = CNode(Array.fill(256)(-1L).updated(1, 13L), 113L, List[Byte](0, 1)).copy(address = 0L)
        val n2 = CNode(Array.fill(256)(-1L), 114L, List[Byte](0, 1, 2, 3))
        val s = newstorage()
        ...
        ns.read(0).get.children.take(49).toList should equal(0 to 48 toList)
        // ns.read(2056).get should equal(n2)
        // ns.read(2057) should equal(None)
      }
    }
  ...
  describe("big") {
    ...
    it("catrie ") {
      val data1 = 0 to 1000000 map (i => 0 to 30 map (j => scala.util.Random.nextPrintableChar) mkString)
      val data = data1.zipWithIndex
        .map(x => x._1.getBytes("US-ASCII").toSeq -> x._2.toLong)
